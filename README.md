...
Didasync 确保您的任务始终保持最新，无论是在 Obsidian 还是在滴答清单中。
...
Didasync 可在桌面端开启本地 HTTP MCP 服务，让支持 MCP 的 AI 插件安全调用任务工具。
...
| ⚡ **创建/插入任务** | 支持在项目中创建任务，或在编辑器中快速插入、关联已有滴答任务 | 使用命令创建/插入，或者[Obsidian 原生任务语法 `- [ ]`](#obsidian-原生任务语法--)创建并同步 |
...
| ✋ **交互与拖拽排期**| 时间段视图支持全天任务拖拽排期| 在时间段/时间块视图里直接拖拽任务调整时间 |
| ↔️ **拖拽跨项目移动** | 支持在任务列表中直接拖拽任务到其他项目，实现项目间移动 | 在侧边栏任务列表里将任务拖到目标项目标题或容器 |
| ✅ **已完成任务视图** | 支持独立查看已完成任务，按时间范围筛选，并可将任务恢复为未完成 | 在任务列表搜索框筛选菜单中打开 `已完成任务` |
...
| 🤖 **MCP / AI 插件集成** | 可开启本地 MCP 服务，让支持 MCP 的 AI 插件通过工具操作滴答任务 | [MCP插件使用](#mcp--ai-插件使用),[效果展示](#YOLO调用MCP的效果)|

## 快速开始
1. [安装并启用插件](#安装) 
2. 在插件设置中使用安全便捷的 OAuth 授权一键连接您的滴答清单/TickTick 账号（无需繁琐配置 API Key）
3. 打开侧边栏或使用功能条图标开始同步您的任务！

## 常见用法

### 查看和恢复已完成任务

1. 在侧边栏任务列表顶部的搜索框筛选菜单中点击 `已完成任务`
2. 在弹出的独立界面中选择开始日期和结束日期
3. 点击 `查询` 获取该时间范围内的已完成任务
4. 如需把某个任务恢复为未完成，直接点击该任务右侧的 `恢复`

### 同步今日任务到当日日记

当前插件支持把今天的任务同步到当日日记中，常见用法如下：

1. 打开 **设置 → Didasync → 同步设置 → 日记同步设置**
...
`8080` 端口被占用时，通常会导致本地 OAuth 回调服务无法启动。此时可以在 **设置 → Didasync → OAuth 配置 → 服务器端口** 中改成其他可用端口，再重新配置一次 redirect URL 后重试。

## MCP / AI 插件使用

在 **设置 → Didasync → 高级/重置 → MCP 服务** 中启用 MCP 服务后，可将以下配置添加到支持 MCP 的 AI 插件：
...
1.4.x 版本引入并扩展了本地 HTTP MCP server，目前暴露了 12 个可供 AI 直接调用的工具：

| 分类 | 工具 |
|---------|-------------|
| 读取工具 | `dida_list_tasks`、`dida_get_task`、`dida_search_tasks`、`dida_list_projects`、`dida_list_completed_tasks` |
| 写入工具 | `dida_create_task`、`dida_update_task`、`dida_schedule_tasks`、`dida_complete_task`、`dida_delete_task`、`dida_move_task` |
| 同步工具 | `dida_sync_now` |

- `dida_list_tasks` 只负责未完成任务
- `dida_list_completed_tasks` 只负责已完成任务，并支持按时间范围筛选
- `dida_move_task` 专门负责跨项目移动任务
...
| 用户发起“帮我安排一下今日任务”，Agent 拉取任务并生成待安排列表。 | Agent 在确认前给出今晚排程方案，等待用户明确“写入滴答”。 |

| 初步排程草案 | 最终写回结果 |
|:--:|:--:|
| ![YOLO MCP Step 2](./assets/yolo-mcp-step-2.png) | ![YOLO MCP Step 4](./assets/yolo-mcp-step-4.png) |
| Agent 根据可用时间、任务时长和优先级输出初步时间块安排。 | Agent将排程写回，显示最终同步后的任务时段。 |
...
2. 创建文件夹：`<vault>/.obsidian/plugins/Didasync/`
3. 将文件复制到该文件夹中，并在 Obsidian 设置中启用插件

## 发布与隐私说明

- 本插件需要用户自行提供并授权滴答清单/TickTick OAuth 账号后才能工作。
- 插件会通过网络访问滴答清单/TickTick 官方接口以读取、创建、更新、完成、删除和同步任务。
- 插件默认不会上传遥测数据，也不包含广告。
- 启用 MCP 服务后，插件会仅在本机 `127.0.0.1` 上启动 HTTP 服务，并使用你配置的 token 进行鉴权。
- OAuth token、MCP token 和插件设置通过 Obsidian 插件数据机制保存在本地 vault 配置目录中。

## 支持

如果您觉得 Didasync 对您有所帮助，请考虑为仓库点个 Star 或提交 Issue 以帮助改进！
