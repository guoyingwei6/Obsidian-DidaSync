...
| 在移动端窄布局下仍可顺畅查看项目分组、任务详情和侧边栏内联添加入口。 | 通过轻量弹窗把项目选择、全天开关和日期选择放在同一个流程里，减少临时记录任务的操作成本。 |
...
| 📱 **支持移动端** | 让项目分组、快速添加入口和任务详情在移动端窄布局中依然易于操作。 | 在移动端打开任务侧边栏，直接按项目分组浏览和管理任务。 |
| ⚡ **创建 / 插入任务** | 在项目中创建任务，或从编辑器插入并关联已有 Dida 任务。 | 使用命令，或通过 Obsidian 原生任务语法创建。 |
| 🗓️ **快速添加弹窗** | 用一个紧凑弹窗完成项目选择、全天设置和日期选择。 | 点击侧边栏里的 `+` 按钮，或运行 `插入/创建滴答任务` 命令。 |
...
| 👁️ **清单显示过滤** | 可隐藏与当前仓库无关的清单，侧边栏只显示需要关注的项目。 | 在同步设置中批量开关，或右键项目标题隐藏/显示。 |
...
![快速添加任务弹窗](./assets/freeaddtask.png)

快速添加弹窗把项目选择和日期选择合并在一起，临时记录任务时不需要在多个对话框之间来回切换。
