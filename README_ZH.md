...
在移动端使用手动 OAuth 流程完成认证：打开认证链接后，从浏览器地址栏复制返回的 `code` 并粘贴回插件。
...
| 📲 **移动端认证支持** | 当移动端浏览器无法自动回到本地回调页时，支持手动完成 OAuth 认证。 | 将重定向 URI 配到滴答开发者后台，打开认证链接后，把返回的 `code` 粘贴回插件。 |
...
![移动端认证支持](./assets/mobileOauth.png)

在移动端上，DidaSync 支持手动 OAuth 认证：授权完成后，直接从浏览器地址栏复制 `code` 参数并粘贴回插件，即可完成认证。
