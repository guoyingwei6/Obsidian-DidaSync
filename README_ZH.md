...
| 📝 **日记集成** | 将今天的任务同步到当日日记。 | 见 [同步今日任务到日记](#同步今日任务到日记)。 |
| 🤖 **MCP / AI 集成** | 暴露本地 MCP 服务，供 AI 插件操作 Dida 任务。 | 见 [MCP / AI 插件使用](#mcp--ai-插件使用)。 |

## 快速开始

1. [安装并启用插件](#安装)。
2. 在插件设置中完成 Dida365 或 TickTick 账号的 OAuth 授权。
...
2. 设置任务写入目标区块的标识或标题。
3. 打开命令面板并运行 `同步今日任务到日记`。
4. DidaSync 会把今天的任务写入当前日记的对应区块。
...
先从 Community Plugins 安装 BRAT，再选择 `Add beta plugin`，粘贴当前仓库地址即可。
...
- 插件需要用户自行提供并授权 Dida365 或 TickTick 账号。
