...
var McpServerManager = class {
  ...
  listTasks(args) {
    ...
    if (args == null ? void 0 : args.projectName) {
    ...
    if (typeof (args == null ? void 0 : args.priority) === "number")
      tasks = tasks.filter((t) => Number(t.priority || 0) === Number(args.priority));
    if (typeof (args == null ? void 0 : args.isAllDay) === "boolean")
      tasks = tasks.filter((t) => !!t.isAllDay === args.isAllDay);
    const datePreset = this.normalizeEnum(args == null ? void 0 : args.datePreset, ["overdue", "today", "tomorrow", "this_week", "scheduled", "unscheduled"], "datePreset");
    const dateField = this.normalizeEnum(args == null ? void 0 : args.dateField, ["startDate", "dueDate", "either"], "dateField") || "either";
    const range = this.resolveDateRange(args == null ? void 0 : args.from, args == null ? void 0 : args.to, datePreset);
    if (datePreset === "scheduled")
      tasks = tasks.filter((t) => !!(t.startDate || t.dueDate));
    if (datePreset === "unscheduled")
      tasks = tasks.filter((t) => !(t.startDate || t.dueDate));
    if (range)
      tasks = tasks.filter((t) => this.taskMatchesDateRange(t, dateField, range.from, range.to));
    const sortBy = this.normalizeEnum(args == null ? void 0 : args.sortBy, ["date", "priority", "updatedAt", "createdAt", "title"], "sortBy") || "date";
    const sortDirection = this.normalizeEnum(args == null ? void 0 : args.sortDirection, ["asc", "desc"], "sortDirection") || "asc";
  ...
  async scheduleTasks(args) {
    if (!Array.isArray(args == null ? void 0 : args.items) || args.items.length === 0)
      throw new Error("items is required and must be a non-empty array");
    const updated = [];
    const errors = [];
    const shouldSync = (args == null ? void 0 : args.sync) !== false;
    for (const [index, item] of args.items.entries()) {
      try {
        if (!item || typeof item !== "object")
          throw new Error("item must be an object");
        if (item.startDate === void 0 && item.dueDate === void 0)
          throw new Error("startDate or dueDate is required for each schedule item");
        ...
        errors.push({ index, id: item == null ? void 0 : item.id, didaId: item == null ? void 0 : item.didaId, message: (e == null ? void 0 : e.message) || "Failed to schedule task" });
      }
    }
    if (updated.length > 0) {
      await this.plugin.saveSettings();
  ...
  serializeTask(task, dateField = "either") {
    return {
      id: task.id,
      didaId: task.didaId,
      title: task.title,
  ...
  taskMutationProperties(includeRequestId) {
    ...
    if (includeRequestId)
      properties.requestId = { type: "string", description: "Idempotency key for retries." };
    return properties;
  }
  normalizeEnum(value, allowed, name) {
  ...
  resolveDateRange(fromArg, toArg, datePreset) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const tomorrow = new Date(today);
    tomorrow.setDate(today.getDate() + 1);
    if (datePreset === "overdue")
      return { from: new Date(0), to: new Date(today.getTime() - 1) };
    if (datePreset === "today")
      return this.dayRange(today);
    if (datePreset === "tomorrow")
      return this.dayRange(tomorrow);
    ...
    return {
      ...
      to: toArg ? this.parseDateBoundary(String(toArg), true) : new Date(864e13)
    };
  }
  dayRange(date) {
  ...
  taskMatchesDateRange(task, dateField, from, to) {
    const dates = dateField === "either" ? [task.startDate, task.dueDate] : [task[dateField]];
  ...
  validateTaskDates(args) {
    if (!args)
      return;
    if (args.startDate !== void 0)
      this.validateDateValue(args.startDate, "startDate");
    if (args.dueDate !== void 0)
      this.validateDateValue(args.dueDate, "dueDate");
    if (args.startDate && args.dueDate) {
      ...
      if (!isNaN(start) && !isNaN(due) && due < start)
        throw new Error("dueDate must be after or equal to startDate");
    }
  }
  validateDateValue(value, name) {
    if (value === null || value === "")
      return;
    const date = new Date(value);
    if (isNaN(date.getTime()))
      throw new Error(`${name} must be a valid date string, for example 2026-05-25 or 2026-05-25T09:00:00+08:00`);
  }
      ...
      if (tasks.length === 0 && this.plugin.getProjectCatalog().length === 0) {
        ...
        for (const [projectName, projectTasks] of sortedProjects) {
          ...
          projectHeader.addEventListener("drop", (e) => {
            ...
            if (e.dataTransfer) {
              ...
              if (draggedTaskId) {
                ...
                if (draggedTask) {
                  void this.plugin.moveTaskToProject(draggedTask, projectInfo.id).then(() => new import_obsidian5.Notice(`\u4EFB\u52A1\u5DF2\u79FB\u52A8\u5230 ${projectInfo.name}`)).catch((error) => new import_obsidian5.Notice((error == null ? void 0 : error.message) || "\u79FB\u52A8\u4EFB\u52A1\u5931\u8D25"));
                }
                return;
              }
          ...
          tasksContainer.addEventListener("dragover", (e) => {
            ...
            e.preventDefault();
            e.stopPropagation();
            if ((_a = e.dataTransfer) == null ? void 0 : _a.types.includes("application/x-dida-task")) {
              if (e.dataTransfer)
                e.dataTransfer.dropEffect = "move";
              projectHeader.classList.add("drag-over");
            }
          ...
          tasksContainer.addEventListener("drop", (e) => {
            ...
            e.stopPropagation();
            projectHeader.classList.remove("drag-over");
            const draggedTaskId = (_a = e.dataTransfer) == null ? void 0 : _a.getData("application/x-dida-task");
            if (!draggedTaskId)
              return;
            const draggedTask = (this.plugin.settings.tasks || []).find((item) => item.id === draggedTaskId || item.didaId === draggedTaskId);
            if (!draggedTask)
...
// src/modals/CompletedTasksModal.ts
...
var CompletedTasksModal = class extends import_obsidian8.Modal {
  ...
  async runQuery() {
    ...
    try {
      ...
      new import_obsidian8.Notice((e == null ? void 0 : e.message) || "\u83B7\u53D6\u5DF2\u5B8C\u6210\u4EFB\u52A1\u5931\u8D25");
    }
  }
  async renderResults(tasks) {
    if (!this.resultEl)
      return;
    this.resultEl.empty();
    if (!Array.isArray(tasks) || tasks.length === 0) {
...
var ProjectCreateModal = class extends import_obsidian9.Modal {
  ...
  submit() {
    ...
    if (!value) {
      new import_obsidian9.Notice("\u9879\u76EE\u6807\u9898\u4E0D\u80FD\u4E3A\u7A7A");
...
var ProjectDeleteConfirmModal = class extends import_obsidian10.Modal {
...
var ProjectIconPickerModal = class extends import_obsidian11.Modal {
  ...
  onOpen() {
    ...
    const renderIcons = (keyword = "") => {
      ...
      this.filteredIcons.forEach((name, index) => {
        ...
        try {
          (0, import_obsidian11.setIcon)(preview, `lucide-${name}`);
        } catch (e) {
